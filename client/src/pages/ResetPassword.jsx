...
const ResetPassword = () => {
  ...
  const handlePaste = (e) => {
    ...
    const paste = e.clipboardData.getData("text").replace(/\D/g, ""); // Allow only digits
    const pasteArray = paste.slice(0, 6).split(""); // Take first 6 digits
    ...
    if (pasteArray.length > 0 && inputRefs.current[pasteArray.length - 1]) {
      inputRefs.current[pasteArray.length - 1].focus(); // Move focus to the last pasted digit
    }
  };
  ...
  const onSubmitEmail = async (e) => {
    ...
    try {
      const { data } = await axios.post(backendUrl + "/api/finance/send-reset-otp", { email });
  ...
  const onSubmitNewPassword=async(e)=>{
    e.preventDefault();
    try{
     const {data}=await axios.post(backendUrl+'/api/finance/reset-password',{email,otp,newPassword})
     data.success?toast.success(data.message):toast.error(data.message)
